...
bool Board::givesCheck(Move m) {
	...
	if (moveType == move::CASTLING) {
		occ.clear(from);
		Square rsq = sideToMove == WHITE ? to > from ? F1 : D1 : to > from ? F8 : D8;
...
bool Board::isPseudoLegal(Move m) {
	...
	if (pt == KNIGHT) {
		...
		int pawnPush = direction::pawnPush(us);
...
void Board::generateKingAttackInfo(KingAttackInfo& k) {
	...
	Color them = !sideToMove;
	Square ksq = this->ksq(sideToMove);
	Bitboard ourTeam = color(sideToMove);
...
bool Board::isLegal(Move m) {
	...
	if (pc == piece::make(us, KING)) {
		if (moveType == move::CASTLING) {
			...
			Bitboard occCopy = occupiedBB;
			PieceType captured = pieceType::make(pieceOn(to));
