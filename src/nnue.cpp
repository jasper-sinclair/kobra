...
alignas(64) int32_t nnue::hidden1_biases[32];
alignas(64) int32_t nnue::hidden2_biases[32];
alignas(64) weight_t nnue::hidden1_weights[64 * 512];
alignas(64) weight_t nnue::hidden2_weights[64 * 32];
...
void nnue::refresh_accumulator(
  ...
  active_indices[0].size = 0;
  active_indices[1].size = 0;
  append_active_indices(pos,active_indices);
  constexpr size_t tile_size = num_regs * simd_width / 16;
  for (unsigned c = 0; c < 2; ++c){
    for (size_t i = 0; i < k_half_dimensions / tile_size; ++i){
      const size_t base = i * tile_size;
      const auto ft_biases_tile = reinterpret_cast<const vec16_t*>(&ft_biases[base]);
      const auto acc_tile = reinterpret_cast<vec16_t*>(&accumulator->accumulation[c][base]);
      ...
      for (size_t k = 0; k < active_indices[c].size; ++k){
        ...
        const size_t offset = static_cast<size_t>(k_half_dimensions) * index + base;
        const auto column = reinterpret_cast<const vec16_t*>(&ft_weights[offset]);
        for (unsigned j = 0; j < num_regs; ++j)
          acc[j] = _mm256_add_epi16(acc[j],column[j]);
      }
      for (unsigned j = 0; j < num_regs; ++j)
        acc_tile[j] = acc[j];
    }
...
bool nnue::update_accumulator(
  ...
  for (size_t i = 0;
       i < k_half_dimensions / tile_size;
       ++i){
    const size_t base = i * tile_size;

    for (unsigned c = 0; c < 2; ++c){
      const auto acc_tile =
        reinterpret_cast<vec16_t*>(
          &accumulator->accumulation[c][base]);

      vec16_t acc[num_regs];

      if (reset[c]){
        const auto ft_b_tile =
          reinterpret_cast<const vec16_t*>(&ft_biases[base]);

        for (unsigned j = 0; j < num_regs; ++j)
          acc[j] = ft_b_tile[j];
      } else{
        const auto prev_acc_tile =
          reinterpret_cast<const vec16_t*>(
            &prev_acc->accumulation[c][base]);

        for (unsigned j = 0; j < num_regs; ++j)
          acc[j] = prev_acc_tile[j];

        for (size_t k = 0; k < removed_indices[c].size; ++k){
          const unsigned index =
            removed_indices[c].values[k];

          const size_t offset =
            static_cast<size_t>(k_half_dimensions) * index + base;

          const auto column =
            reinterpret_cast<const vec16_t*>(&ft_weights[offset]);

          for (unsigned j = 0; j < num_regs; ++j)
            acc[j] = _mm256_sub_epi16(acc[j],column[j]);
        }
      }

      for (size_t k = 0; k < added_indices[c].size; ++k){
        const unsigned index =
          added_indices[c].values[k];
...
inline void nnue::append_active_indices(
  const nnboard* pos,
  index_list active[2]){
...
inline void nnue::append_changed_indices(
  ...
  const dirty_piece* dp = &pos->nnue[0]->dirty;
  if (pos->nnue[1] && pos->nnue[1]->accumulator.computed_accumulation){
    ...
    const dirty_piece* dp2 = &pos->nnue[1]->dirty;
    for (int c = 0; c < 2; c++){
...
inline void nnue::half_kp_append_active_indices(
  const nnboard* pos,
  const int color,
  index_list* active){
  const unsigned ksq = orient(color,pos->squares[color]);

  for (int i = 2; pos->pieces[i]; i++){
    const int sq = pos->squares[i];
    const int pc = pos->pieces[i];
    active->values[active->size++] =
      make_index(color,sq,pc,ksq);
  }
}
...
inline void nnue::half_kp_append_changed_indices(
  ...
  for (int i = 0; i < dp->dirty_num; i++){
    ...
    if (dp->from[i] != 64)
      removed->values[removed->size++] =
        make_index(color,dp->from[i],pc,ksq);

    if (dp->to[i] != 64)
      added->values[added->size++] =
        make_index(color,dp->to[i],pc,ksq);
  }
}

unsigned nnue::make_index(
...
inline const char* nnue::read_hidden_weights(
  weight_t* w,
  const unsigned dims,
  const char* d){
...
inline void nnue::permute_biases(
  int32_t* biases){
...
inline uint32_t nnue::readu_le_u32(
  const void* p){
...
inline int16_t nnue::readu_le_u16(
  const void* p){
  const auto q = static_cast<const uint8_t*>(p);
  return static_cast<int16_t>(q[0] | q[1] << 8);
}

inline void nnue::read_output_weights(
  weight_t* w,
  const char* d){
